<template>
  <section class="py-[60px] max-md:py-10" id="blog">
    <div class="max-w-[1200px] mx-auto px-5">
      <div class="flex justify-between items-end mb-9">
        <div>
          <h2 class="font-display text-[clamp(28px,4vw,40px)] tracking-[2px]">TIN TỨC & PHONG CÁCH</h2>
          <p class="text-[13px] text-text-muted mt-1 tracking-[0.5px]">Xu hướng, tips mix đồ và tin tức sneaker mới nhất</p>
        </div>
        <a href="#" class="text-[12px] text-accent tracking-[1.5px] uppercase flex items-center gap-[6px] transition-all whitespace-nowrap font-medium hover:gap-[10px]">Tất cả bài viết <i class="ti ti-arrow-right"></i></a>
      </div>
      ...
      <div class="grid grid-cols-3 max-md:grid-cols-1 gap-5">
        ...
        >
          <div class="h-[180px] bg-surface2 flex items-center justify-center text-[60px] relative overflow-hidden">
            ...
            <span :class="['absolute top-3 left-3 text-[9px] tracking-[1.5px] uppercase py-1 px-2.5 rounded-sm z-[2]', post.tagClass]">{{ post.tag }}</span>
          </div>
          <div class="p-[18px]">
            <div class="text-[11px] text-text-dim mb-2 tracking-[0.5px]">{{ post.date }}</div>
            <h4 class="text-[15px] font-medium leading-[1.4] mb-2.5 text-text line-clamp-2">{{ post.title }}</h4>
            ...
            <div class="flex items-center justify-between pt-3 border-t border-border">
              ...
              <button class="flex items-center gap-[5px] bg-transparent border-none text-[12px] cursor-pointer transition-colors" :style="{ color: post.liked ? 'var(--tw-color-accent, #FF4D00)' : '' }" @click="likePost(post)">
...
const posts = reactive([
  ...
  { id: 3, icon: '👟', tag: 'Xu Hướng', tagClass: 'bg-accent2 text-black', date: '02 Tháng 5, 2026', title: '5 đôi sneaker đang hot nhất 2026 mà mọi tín đồ streetwear phải có', excerpt: 'Từ Adidas Samba đến Nike Dunk Low — đây là những đôi sneaker đang được săn đón nhiều nhất trong năm nay.', author: 'Khánh', likes: 201, liked: false },
])
